public class PeonB {
    public static String[] main (String arg) {
        ...
        if (numero == 7) { //Si está en la fila 1 no puede moverse por que es una posición imposible para un peón blanco
            result[0] = "X";
            result[1] = "X";
        } else if (numero == 6) { //Si está en la fila 2 puede moverse 2 posiciones, como al empezar una partida
            result[0] = matriz[numero - 1][valorLetra];
            result[1] = matriz[numero - 2][valorLetra];
        } else if (numero == 0) { //Si la fila es la 8 no puede moverse a ninguna parte
            result[0] = "X";
            result[1] = "X";
        } else { //En el resto de casos podrá moverse una casilla hacia arriba
            result[0] = matriz[numero - 1][valorLetra];
            result[1] = "X";
        }
